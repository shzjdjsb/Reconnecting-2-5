...
class Evaluator(object):
    def __init__(self, num_class, ignore_index=None, exclude_classes=None):
    ...
    def Precision(self):
        ...
        precision = tp / (tp + fp)
        return self._exclude(precision)

    def Recall(self):
        tp, fp, tn, fn = self.get_tp_fp_tn_fn()
        recall = tp / (tp + fn)
        return self._exclude(recall)
    ...
    def _exclude(self, values):
        values = np.asarray(values, dtype=float).copy()
        for class_id in self.exclude_classes:
            if 0 <= class_id < self.num_class:
                values[class_id] = np.nan
        return values
